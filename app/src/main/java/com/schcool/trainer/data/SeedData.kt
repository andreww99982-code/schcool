...
object SeedData {
    ...
    val sellerQuestionTemplates = listOf(
        "Для каких задач вы чаще всего используете смартфон?",
        "Что для вас важнее: камера, батарея или производительность?",
        "Какой бюджет для вас комфортный?",
        "Вы чаще снимаете фото днём или в сложном освещении?",
        "Нужна ли вам защита от воды и пыли?",
        "Сколько памяти вам обычно не хватает на текущем телефоне?",
        "Вы играете в тяжёлые игры или больше рабочие задачи?",
        "Нужна ли вам eSIM или классическая SIM-карта?",
        "Насколько важна быстрая зарядка в вашем сценарии?",
        "Вы рассматриваете покупку в рассрочку или сразу оплату?",
        "Есть ли предпочтение по бренду или открыты к вариантам?",
        "Планируете использовать телефон 2–3 года без обновления?",
        "Важна ли вам видеосъёмка или в приоритете фото?",
        "Какой размер экрана для вас удобен в повседневности?",
        "Нужна ли интеграция с другими вашими устройствами?",
        "Готовы рассмотреть две модели для сравнения перед выбором?"
    )
}
